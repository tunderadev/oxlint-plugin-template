...
Releases are manual and have no schedule. One is due when main has something a user would notice since the last tag: a `feat` or `fix` commit, or a change to a runtime dependency. A workflow checks for that on every push to main and opens a "release pending" issue that names the bump to make. Dev dependency bumps, docs, and CI changes never need a release on their own.
